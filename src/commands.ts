...
async function handleDelete(
  ...
  if (args) {
    ...
    const targetArg = (confirm ? parts.slice(0, -1).join(" ") : args).trim()
    // Bulk wipe: all | -all | --all | * (confirm required, like cross-chat deletes).
    if (["all", "-all", "--all", "*"].includes(targetArg.toLowerCase())) {
      const list = await router.listSessions()
      if (list.length === 0) return { handled: true, text: "No sessions to delete." }
      if (!confirm) {
        return { handled: true, text: `This will delete ${list.length} session(s). Resend as \`${prefix} all confirm\` to confirm.` }
      }
      let deleted = 0
      let failed = 0
      for (const s of list) {
      ...
      for (const id of router.allChatIds()) router.detachChat(id)
      try {
        const fresh = await router.createForChat(chatJid, undefined, model)
        const tail = failed > 0 ? ` ${failed} failed (may still show as unmapped).` : ""
        return { handled: true, text: `Deleted ${deleted} session(s), started fresh session ${fresh.sessionId.slice(0, 8)}.${tail}` }
      } catch {
        return { handled: true, text: `Deleted ${deleted} session(s). Could not start a fresh one (opencode unreachable) — your next message will create it.` }
      }
    }
    ...
    if (cur?.sessionId !== target && !confirm) {
      return { handled: true, text: `That session (${target.slice(0, 8)}) is not this chat's. Resend as \`${prefix} ${targetArg} confirm\` to delete it.` }
    }
...
export function helpText(): string {
  return [
    ...
    "  /session delete [<n>|all] [confirm] delete session(s) (confirm if not this chat's / all)",
    ...
    "  /delete     delete the current session for this chat (alias for /session delete)",
    "  /delete <n> delete another session by /sessions number (needs confirm if not this chat's)",
...
export async function handleCommand(
  ...
  switch (lower) {
    ...
    case "/session": {
      if (!args) return { handled: true, text: "Usage: /session <id|[n]|delete [<n>|all] [confirm]>\nGet numbers from /sessions." }
      const [sub, ...subRest] = args.split(/\s+/)
      if (sub?.toLowerCase() === "delete") {
        return handleDelete(router, client, config, chatJid, subRest.join(" ").trim(), "/session delete")
      }
      let target = args
    ...
    case "/delete": {
      return handleDelete(router, client, config, chatJid, args, "/delete")
    }
